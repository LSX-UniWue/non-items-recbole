...
def apply_min_item_feedback(min_item_feedback, page_views):
    aggregated = page_views.groupby(['movieId']).size()
    filtered = aggregated.apply(lambda v: v >= min_item_feedback)
...
def apply_min_sequence_length(min_sequence_length, dataset):
    aggregated = dataset.groupby(['userId']).size()
    filtered = aggregated.apply(lambda v: v >= min_sequence_length)
    filtered = filtered.reset_index()
...
def data_ratio_split(pd_data, train_percent: float = 0.8, val_percent: float = 0.1, test_percent: float = 0.1, min_sequence_length=4):
    ...
    test = pd_data[pd_data['userId'].isin(test_users)]

    train = apply_min_sequence_length(min_sequence_length,train)
    validation = apply_min_sequence_length(min_sequence_length,val)
    test = apply_min_sequence_length(min_sequence_length,test)
