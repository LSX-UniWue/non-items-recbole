...
* The [COVEO](https://github.com/coveooss/SIGIR-ecom-data-challenge) dataset has to be downloaded manually from [here](https://www.coveo.com/en/ailabs/sigir-ecom-data-challenge), as it requires registering.
    ...
    * Unzip with e.g. `unzip path/to/SIGIR-ecom-data-challenge.zip -d datasets/raw/coveo/`
    * There should be three files in  `SIGIR-ecom-data-challenge\train`: `browsing_train.csv`, `search_train.csv`, `sku_to_content.csv`
    * Provide your dataset path, e.g. `.datasets/raw/coveo/` in the next step
...
1. `python data_preparation/main.py --dataset ml-20m --random` - SynData and Randomized SynData
2. `python data_preparation/main.py --dataset coveo-search --dataset_path .datasets/raw/coveo/` - Coveo Search Dataset, provide your path to the raw data
3. `python data_preparation/main.py --dataset coveo-pageview --dataset_path .datasets/raw/coveo/` - Coveo Pageview Dataset, provide your path to the raw data
